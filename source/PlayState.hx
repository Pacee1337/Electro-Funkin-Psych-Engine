...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'school': //Week 6 - Warning, Idiot Bot
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'warning' | 'idiot-bot' | 'rise':
					if(daSong == 'idiot-bot') FlxG.sound.play(Paths.sound('ANGRY'));
					schoolIntro(doof);
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var songName:String = Paths.formatToSongPath(SONG.song);
		if (songName == 'idiot-bot' || songName == 'rise')
		{
			remove(black);
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					if (Paths.formatToSongPath(SONG.song) == 'rise')
					{
						add(senpaiEvil);
						senpaiEvil.alpha = 0;
